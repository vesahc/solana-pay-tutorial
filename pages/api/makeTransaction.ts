...
function get(res: NextApiResponse<MakeTransactionGetResponse>) {
    res.status(200).json({
        label: "Ravens Eye Healing",
        icon: "https://freesvg.org/img/Raven_by_Rones.png",
    })
}
...
async function post(
    ...
    try {
        ...
        const transferInstruction = createTransferCheckedInstruction(
            ...
            amount.toNumber() * (10 ** (await usdcMint).decimals), // amount to transfer
        ...
        // The counpon instruction is to send 1 coupon from the shop to the buyer
        createTransferCheckedInstruction(
            shopCouponAddress, // source account (coupon)
            couponAddress, // token address (coupon)
            buyerCouponAccount.address, // owner of source account
        ...
        // We must partial sign because the transfer instruction sill requires the user
        transaction.partialSign(shopKeypair)

        // Serialize the transaction and conver to base64 to return it
